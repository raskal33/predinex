...
library PoolStats {
    ...
    /**
     * @notice Get adjusted fee rate based on user's PRIX balance
     * @param userBalance User's PRIX token balance
    ...
    /**
     * @notice Check if pool uses PRIX token
     * @param flags Pool flags byte
     * @return usesPrix True if pool uses PRIX
     */
    function poolUsesPrix(uint8 flags) internal pure returns (bool usesPrix) {
        return (flags & 0x02) != 0;
    }
