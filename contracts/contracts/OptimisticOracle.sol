...
// Interface for PrixedictPool to interact with
interface IPrixedictPool {
...
contract OptimisticOracle is Ownable {
    ...
    IERC20 public immutable bondToken; // PRIX token for bonds (BNB is native coin)
    IPrixedictPool public immutable prixedictPool;
    
    // Oracle configuration
    uint256 public constant PROPOSAL_BOND = 50e18; // 50 PRIX bond to propose outcome
    uint256 public constant DISPUTE_BOND = 100e18; // 100 PRIX bond to dispute
    ...
    struct Market {
        ...
        uint256 poolId;             // Associated PrixedictPool ID
    ...
    constructor(address _bondToken, address _prixedictPool) Ownable(msg.sender) {
        bondToken = IERC20(_bondToken);
        prixedictPool = IPrixedictPool(_prixedictPool);
    }
    ...
    modifier onlyPrixedictPool() {
        require(msg.sender == address(prixedictPool), "Only PrixedictPool can call");
        _;
    }
    ...
    function resolveMarket(string memory marketId) external {
        ...
        // Settle the pool in PrixedictPool
        prixedictPool.settlePool(market.poolId, market.finalOutcome);
    }

    // Get market outcome (used by PrixedictPool)
    ...
    function emergencyResolveMarket(string memory marketId, bytes32 outcome) external onlyOwner {
        ...
        prixedictPool.settlePool(market.poolId, outcome);
