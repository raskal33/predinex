...
contract PrixedictComboPools is Ownable, ReentrancyGuard {
    ...
    // Constants
    uint256 public constant creationFeeBNB = 1e18;     // 1 BNB
    uint256 public constant creationFeePRIX = 70e18;   // 70 PRIX
    uint256 public constant minPoolStakeBNB = 5e18;    // 5 BNB
    uint256 public constant minPoolStakePRIX = 1000e18; // 1000 PRIX
    ...
    struct ComboPool {
        ...
        uint8 flags;               // Packed bools: bit 0=settled, bit 1=creatorSideWon, bit 2=usesPrix
    ...
    constructor(
        ...
        require(_prixToken != address(0), "Invalid PRIX address");
    ...
    function createComboPool(
        ...
        // Check minimum stake
        if (usePrix) {
            require(creatorStake >= minPoolStakePRIX, "PRIX stake below minimum");
        } else {
            require(creatorStake >= minPoolStakeBNB, "BNB stake below minimum");
        }
        ...
        uint256 creationFee = usePrix ? creationFeePRIX : creationFeeBNB;
        uint256 totalRequired = creationFee + creatorStake;
        
        if (usePrix) {
            require(prixToken.transferFrom(msg.sender, address(this), totalRequired), "PRIX transfer failed");
            totalCollectedPRIX += creationFee;
        } else {
            require(msg.value == totalRequired, "Incorrect BNB amount");
            totalCollectedBNB += creationFee;
        }
    ...
    function placeComboBet(uint256 comboPoolId, uint256 amount) external payable validComboPool(comboPoolId) nonReentrant {
        ...
        // Handle payment
        if (_comboPoolUsesPrix(comboPoolId)) {
            require(prixToken.transferFrom(msg.sender, address(this), amount), "PRIX transfer failed");
        } else {
    ...
    function claimCombo(uint256 comboPoolId) external validComboPool(comboPoolId) nonReentrant {
        ...
        if (_comboPoolCreatorSideWon(comboPoolId)) {
            ...
            if (stake > 0) {
                ...
                if (fee > 0) {
                    if (_comboPoolUsesPrix(comboPoolId)) {
                        totalCollectedPRIX += fee;
                    } else {
                        totalCollectedBNB += fee;
                    }
                }
                
                // Reputation for high-value wins
                uint256 minValueBNB = 10 * 1e18;
                uint256 minValuePRIX = 2000 * 1e18;
                bool qualifiesForReputation = _comboPoolUsesPrix(comboPoolId) ? 
                    (stake >= minValuePRIX) : (stake >= minValueBNB);
        ...
        if (_comboPoolUsesPrix(comboPoolId)) {
            require(prixToken.transfer(msg.sender, payout), "PRIX payout failed");
        } else {
            (bool success, ) = payable(msg.sender).call{value: payout}("");
            require(success, "BNB payout failed");
        }
    ...
    function refundComboPool(uint256 comboPoolId) external validComboPool(comboPoolId) {
        ...
        for (uint256 i = 0; i < lps.length; i++) {
            ...
            if (stake > 0) {
                if (_comboPoolUsesPrix(comboPoolId)) {
                    require(prixToken.transfer(lp, stake), "PRIX LP refund failed");
                } else {
                    (bool success, ) = payable(lp).call{value: stake}("");
                    require(success, "BNB LP refund failed");
                }
            }
        }
        ...
        for (uint256 i = 0; i < bettors.length; i++) {
            ...
            if (stake > 0) {
                if (_comboPoolUsesPrix(comboPoolId)) {
                    require(prixToken.transfer(bettor, stake), "PRIX bettor refund failed");
                } else {
                    (bool success, ) = payable(bettor).call{value: stake}("");
                    require(success, "BNB bettor refund failed");
                }
            }
        }
    ...
    function distributeFees(address stakingContract) external {
        ...
        if (_bnb > 0) {
            uint256 bnbStakers = (_bnb * 30) / 100;
            totalCollectedBNB = 0;
            (bool success1, ) = payable(feeCollector).call{value: _bnb - bnbStakers}("");
            require(success1, "BNB fee collector transfer failed");
            (bool success2, ) = payable(stakingContract).call{value: bnbStakers}("");
            require(success2, "BNB staking transfer failed");
        }
        
        if (_prix > 0) {
            uint256 prixStakers = (_prix * 30) / 100;
            totalCollectedPRIX = 0;
            prixToken.transfer(feeCollector, _prix - prixStakers);
            prixToken.transfer(stakingContract, prixStakers);
        }
    }
    ...
    function _comboPoolUsesPrix(uint256 comboPoolId) internal view returns (bool) {
